...
fn write_time_table(
    ...
    //
    // `>=`, not `>`. A reader treats "compressed length == uncompressed length"
    // as the sentinel for a section stored RAW, so writing compressed bytes
    // when the two are exactly equal makes the time table undecodable: readers
    // skip the inflate and parse zlib's own header as varints. The block, the
    // block chain and every length field still validate, so the file passes
    // structural checks and then yields nonsense timestamps -- gtkwave's
    // fst2vcd "succeeds" and prints times orders of magnitude off. Break-even
    // is not rare: it happens whenever the delta-encoded table is small and
    // incompressible, i.e. a short run with irregular time steps.
